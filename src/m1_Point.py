...
class Point(object):
    ...
    def move_to(self, x, y):
        self.distancetravelled = self.distancetravelled + \
    ...
    def move_by(self, dx, dy):
        self.distancetravelled = self.distancetravelled + \
                                 self.get_distance_from(Point(self.x + dx,
                                                              self.y + dy))
        self.x = self.x + dx
        self.y = self.y + dy
        self.movesmade = self.movesmade + 1
    ...
    def get_distance_from_start(self):
        return math.sqrt((self.x - self.startx) ** 2 + (self.y -
                                                         self.starty)
                         ** 2)
    ...
    def closer_to(self, point2, point3):
        if self.get_distance_from(point2) > self.get_distance_from(point3):
            return point3
        elif self.get_distance_from(point2) < self.get_distance_from(point3):
            return point2
        else:
            return point2
